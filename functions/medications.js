...
medications.hasActiveMed = function(patient, date, medInfo, errorContainer) {
  ...
  var doseMin = dictionary.defaults.doses.min,
  doseMax = dictionary.defaults.doses.max;
...
medications.activeMedMin = function( patient, date, minCount, errorContainer ){
  // Check input
  if( utils.isUndefinedOrNull( patient, date, minCount )){
    return utils.invalid(
      "Invalid data passed to medications activeMedMin",
      errorContainer
    );
  }
